...
const generateSystemInstruction = (): string => {
  // Assemble a markdown formatted list of doctor profiles. Note that doc.name
  // already includes the "Dr." prefix, so we do not prepend it again.
  const doctorProfiles = DOCTORS.map(doc =>
    `- ${doc.name}, Specialty: ${doc.specialty}. Bio: ${doc.bio}`
  ).join('\n');
